...
export type Token =
  ...
  | { type: "command_type"; value: "call" | "normal" }
  ...
  | { type: "named_arg"; key: string; value: string; escaped: boolean }
  | { type: "paren_open" }
  | { type: "paren_close" }
  | { type: "comma" }
  | { type: "semicolon" }
...
export function tokenize(input: string, log: boolean = false): Token[] {
  ...
  for (const segment of segments) {
    ...
    const regex =
      /\s+|(['"])(.*?)\1(?:\([a-zA-Z_][a-zA-Z0-9_]*\))?|"(.*?)"|\d+\([a-zA-Z_][a-zA-Z0-9_]*\)|[\$a-zA-Z_][\$a-zA-Z0-9_\.]*\([a-zA-Z_][a-zA-Z0-9_]*\)|[\$a-zA-Z_][\$a-zA-Z0-9_]*:[\$a-zA-Z_][\$a-zA-Z0-9_\.]*|->|\d+|true|false|\([a-zA-Z_][a-zA-Z0-9_]*\)|[+\-*/%]?=|\(|\)|,|;|\||[\$a-zA-Z_][\$a-zA-Z0-9_\.]*/g;
    ...
    while ((match = regex.exec(seg))) {
      ...
      // Pipe
        ...
        for (const arg of args) {
          ...
          const namedArgMatch =
            /^\\?([a-zA-Z_][a-zA-Z0-9_]*)=(['"]?)(.*?)\2$/.exec(arg.trim());
...
function getCommandType(segment: string): "call" | "normal" {
