...
describe("lessThanOrEqual state function", () => {
  beforeAll(async () => {
    ...
    setupTestFrame(
      /* html */ `
      <span id="el1" data-cspine="lessThanOrEqual:number->2(number)" x-show="$_.state.lessThanOrEqual"></span>
      <span id="el2" data-cspine="lessThanOrEqual:number->0(number)" x-show="$_.state.lessThanOrEqual"></span>
      <span id="el3" data-cspine="lessThanOrEqual:number->1(number)" x-show="$_.state.lessThanOrEqual"></span>
  `,
      data
    );
  });
