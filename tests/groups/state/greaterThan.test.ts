...
describe("greaterThan state function", () => {
  beforeAll(async () => {
    ...
    setupTestFrame(
      /* html */ `
      <span id="el1" data-cspine="greaterThan:number->2(number)" x-show="$_.state.greaterThan"></span>
      <span id="el2" data-cspine="greaterThan:number->6(number)" x-show="$_.state.greaterThan"></span>
  `,
      data
    );
  });
